...
function update_user_info()
{
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            $sql = 'SELECT rank_id, discount,rank_name FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE special_rank = '0' AND min_points <= " . intval($row['rank_points']) . ' AND max_points > ' . intval($row['rank_points']);
            if ($row = $GLOBALS['db']->getRow($sql))
            {
...
$GLOBALS['db'] -> query($sql);
/*$sql1 = "update ".$GLOBALS['ecs']->table('cart')." set session_id ='".md5($_SESSION['user_id'].'_'.$_SESSION['user_name'])."' where user_id = '".$_SESSION['user_id']."'";
...
function get_navigator($ctype = '', $catlist = array())
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
	   ...
	   $catchild = array();
	   if($row['ctype'] == 'c' && $row['cid'] > 0)
       {
          $catchild = get_child_tree($row['cid']);
       }
