...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name, category_logo, measure_unit, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
			   ...
			   $three_arr[$row['cat_id']]['measure_unit'] = $row['measure_unit'];
			   $three_arr[$row['cat_id']]['category_logo'] = $row['category_logo'];
 ...
 * @return  array
 */
function get_goods_properties($goods_id,$selected_spec=array())
{
    /* 对属性进行重新排序和分组 */
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type,g.attr_desc, ".

                "g.goods_attr_id, g.attr_value, g.attr_price,g.attr_gallery_id,g.thumb_url,g.img_original,g.img_url,g.attr_pic " .

            'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
            "WHERE g.goods_id = '$goods_id' " .
            'ORDER BY g.sort_order asc, g.attr_value asc, g.goods_attr_id';
    $res = $GLOBALS['db']->getAll($sql);
    ...
    foreach ($res AS $row)
    {
        //$row['attr_desc'] = str_replace("\n", '<br />', $row['attr_desc']);
		$row['attr_desc'] = trim($row['attr_desc'],"\n");
        ...
        else
        {
			...
			if($row['attr_name']=='定制颜色' || $row['attr_name']=='颜色' || $row['attr_name']=='手机颜色')
			{
				$row['is_type'] = 1;
			}
			if($row['attr_name']=='型号')
			...
			$gallery_info = array();
			$gallery_list = array();
			if(!empty($row['attr_gallery_id']))
			{
			   $attr_gallery_arr = explode(',',$row['attr_gallery_id']);
			   $gallery_list = $GLOBALS['db']->getAll("select img_id,img_url,thumb_url,img_original from " . $GLOBALS['ecs']->table('goods_gallery') . 
			                                          " where img_id " . db_create_in($attr_gallery_arr)."");
			}
			
			
			$gallery_info['thumb_url'] = '';
			$gallery_info['img_original'] = '';
			$gallery_info['img_url'] = '';
			

			if(!empty($row['img_url']))
			{
				$gallery_info['thumb_url'] = $row['thumb_url'];
				$gallery_info['img_original'] = $row['img_original'];
				$gallery_info['img_url'] = $row['img_url'];
				$row['is_self'] = 1;
			}
			...
			{
				$gallery_info['thumb_url'] = $gallery_list[0]['thumb_url'];
				$gallery_info['img_original'] = $gallery_list[0]['img_original'];
				$gallery_info['img_url'] = $gallery_list[0]['img_url'];
			}
			...
			if($row['attr_desc'])
			{
				$row['attr_url'] = build_uri('goods', array('gid' => $row['attr_desc']));
			}
            ...
            $arr['spe'][$row['attr_id']]['attr_type'] = $row['attr_type'];
            $arr['spe'][$row['attr_id']]['name']     = $row['attr_name'];
			$arr['spe'][$row['attr_id']]['is_type']     = $row['is_type'];
			$arr['spe'][$row['attr_id']]['attr_id']     = $row['attr_id'];
            $arr['spe'][$row['attr_id']]['values'][] = array(
                                                        'label'        => $row['attr_value'],
                                                        'price'        => $row['attr_price'],
														'attr_selected'        => $row['attr_selected'],
														'gallery_list' => $gallery_list,
														'thumb_url' =>$gallery_info['thumb_url'],
														'is_self' =>$row['is_self'],
														'img_url' =>$gallery_info['img_url'],
														'img_original' =>$gallery_info['img_original'],
														'is_type' => $row['is_type'],
														//'thumb_url' => $row['is_type']==1 ? $GLOBALS['_LANG'][$row['attr_value']] : '',
                                                        'attr_pic'        => $row['attr_pic'],
														'attr_desc'        => $row['attr_desc'],
														'img_original' =>$gallery_info['img_original'],
														'img_url' =>$gallery_info['img_url'],
														'attr_gallery_id' =>$row['attr_gallery_id'],
...
function get_cat_goods($cat_id=0,$num=8)
{
	
	  $goods = array();
      $children = get_children ($cat_id);
	  $where = " and g.is_on_sale = 1 AND g.is_alone_sale = 1 AND " ."g.is_delete = 0 AND ($children OR " . get_extension_goods ( $children ) . ')';
	  $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
			  "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
			  "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, g.original_img, color,color_bg " .
			  'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
			  "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
			  "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
	  $sql .= " where 1 $where ORDER BY g.sort_order, g.goods_id DESC";
	  $result = $GLOBALS['db']->getAll($sql);
	  ...
	  foreach ($result AS $idx => $row)
	  {
		  ...
		  {
			  $goods[$row['goods_id']]['promote_price'] = '';
		  }
		  ...
		  $goods[$row['goods_id']]['color']         = $row['color'];
		  $goods[$row['goods_id']]['color_bg']         = $row['color_bg'];
		  $goods[$row['goods_id']]['brief']        = $row['goods_brief'];
		  
		  $goods[$row['goods_id']]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
										 sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
		  ...
		  $goods[$row['goods_id']]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img'],true);
		  $goods[$row['goods_id']]['original_img']    = get_image_path($row['goods_id'], $row['original_img'],true);
		  
		  $goods[$row['goods_id']]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		  $goods[$row['goods_id']]['short_style_name'] = add_style($goods[$row['goods_id']]['short_name'], $row['goods_name_style']);
   }
   
	 $sql = "SELECT goods_id,img_id,img_url,thumb_url,img_original,img_desc " .
		  'FROM ' . $GLOBALS['ecs']->table('goods_gallery') . ' AS g ' .
		  " WHERE goods_id " . db_create_in($goods_ids) . " and img_desc=1";
	 $res = $GLOBALS['db']->getAll($sql);
	  ...
	  {
		  $goods[$row['goods_id']]['goods_gallery'][$key]['img_desc'] = $row['img_desc'];
		  $goods[$row['goods_id']]['goods_gallery'][$key]['img_url'] = $row['img_url'];
		  $goods[$row['goods_id']]['goods_gallery'][$key]['thumb_url'] = $row['thumb_url'];
		  $goods[$row['goods_id']]['goods_gallery'][$key]['img_original'] = $row['img_original'];
	  }
