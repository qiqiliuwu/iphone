...
function order_paid($log_id, $pay_status = PS_PAYED, $note = '')
{
    ...
    if ($log_id > 0)
    {
        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            ...
            {
                /* 取得订单信息 */
                $sql = 'SELECT order_id, user_id, order_sn, consignee, address,mobile, tel, shipping_id, extension_code, extension_id, goods_amount ' .
...
global $_CFG;
				...
				if ($GLOBALS['_CFG']['ecsdxt_order_payed'] == '1' && $GLOBALS['_CFG']['ecsdxt_shop_mobile'] != '')
				{
					...
					$content = $smarty->fetch('str:' . $GLOBALS['_CFG']['ecsdxt_order_payed_value']);
				...
				/* 客户付款时给客户发送短信提醒 */
				if ($GLOBALS['_CFG']['ecsdxt_customer_payed'] == '1' && $order['mobile'] != '')
				{
					require_once(ROOT_PATH . 'includes/lib_sms.php');
					...
					$content = $smarty->fetch('str:' . $GLOBALS['_CFG']['ecsdxt_customer_payed_value']);
