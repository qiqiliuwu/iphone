...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, credit_line, last_login, is_validated, rank_points, user_rank, photo FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
	...
	if ($row['user_rank'] == 0)
	{
		...
		$sql = 'SELECT rank_id, discount,rank_name FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE min_points <= " . intval($row['rank_points']) . ' AND max_points > ' . intval($row['rank_points']);
		if ($row1 = $GLOBALS['db']->getRow($sql))
		{
			$info['user_rank'] = $row1['rank_id'];
            $info['rank_name'] = $row1['rank_name'];
			$info['discount'] = $row1['discount'];
		}
		else
		{
			$info['user_rank'] = 0;
	...
	{
		// 特殊等级
		$sql = 'SELECT rank_id, discount,rank_name FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_id = '$row[user_rank]'";
		if ($row1 = $GLOBALS['db']->getRow($sql))
		{
			$info['user_rank'] = $row1['rank_id'];
            $info['rank_name'] = $row1['rank_name'];
	...
	$info['mobile_verify']  = $row['mobile_verify'];
	$info['free_points']  = $row['free_points'];
