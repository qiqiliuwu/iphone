...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    foreach ($arr as $key => $value)
    {
		...
		$total['discount_format'] = price_format($discount,false);
		$total['amount'] = $total['goods_price']-$discount;
		$total['amount_format']  = price_format($total['amount'], false);*/
...
function get_cart_goods()
{
    ...
    $total['will_get_integral'] = get_give_integral();
...
function get_tb_num($goods_id,$goods_sn) {
		$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('sharegoods_module') . " WHERE class = 'taobao'";		
		$tb = $GLOBALS['db']->getRow($sql);
		$data = unserialize($tb['api_data']);
		
		$app_key = $data['app_key'];
		$app_secret = $data['app_secret'];

		include_once ROOT_PATH.'sdks/taobao/TopClient.php';
        include_once ROOT_PATH.'sdks/taobao/request/ItemGetRequest.php';
		...
		$req->setFields("detail_url,num_iid,title,nick,type,cid,seller_cids,props,input_pids,input_str,desc,pic_url,num,prop_imgs,valid_thru,list_time,delist_time,stuff_status,
		location,price,post_fee,express_fee,ems_fee,item_weight,item_size,has_discount,freight_payer,has_invoice,has_warranty,has_showcase,
		modified,increment,approve_status,postage_id,product_id,auction_point,props_name,property_alias,item_img,prop_img,sku,video,outer_id,is_virtual");
    	$req->setNumIid($goods_sn);
	...
	$sql = "SELECT pro_id ,product_number 
    ...
    foreach ($row as $value)
    {
			...
			for($i=0;$i<count($sku);$i++) {
				...
				$properties = (array)$sku[$i];
	
				$pro_s = explode(';',$properties['properties_name']);
				
				if (!empty($pro_s[0])) {
					$chars = get_pro_a($pro_s[0],3); 
					$char = $chars[0];}
				if (!empty($pro_s[1])) {
					$chars1 = get_pro_a($pro_s[1],3); 
					$char = $char.':'.$chars1[0];}
				if (!empty($pro_s[2])) {
					$chars3 = get_pro_a($pro_s[2],3); 
					$char = $char.':'.$chars3[0];}
					
				if(is_int(strpos($pro_id,$char))){

					    $quantity = $properties['quantity'];
						$sql = "UPDATE " . $GLOBALS['ecs']->table('products') .
							" SET product_number = '$quantity' " .
							"WHERE goods_id = '$goods_id' and pro_id = '$pro_id'";
					$GLOBALS['db']->ping();
					$GLOBALS['db']->query($sql);					
					break;
				}
    ...
    $sql = "SELECT SUM(product_number) FROM " . $GLOBALS['ecs']->table('products') .
        " WHERE goods_id='$goods_id'";

    return $GLOBALS['db']->getOne($sql);

}
function get_pro_a($str,$num){

	$str1=preg_replace('/:/','-',$str,1);
	$chars1 =preg_split('/:/', $str1, $num, PREG_SPLIT_NO_EMPTY);
	$chars1[0]=preg_replace('/-/',':',$chars1[0]);
	return $chars1;
}
?>