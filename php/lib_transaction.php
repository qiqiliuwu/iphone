...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT photo,user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
...
function get_consignee_list($user_id)
{
    ...
    $sql = "SELECT u.*,r.region_name as country_name,r1.region_name as province_name,r2.region_name as city_name,r3.region_name as district_name FROM " . $GLOBALS['ecs']->table('user_address') . " as u ".
	        " left join " . $GLOBALS['ecs']->table('region') . " as r on r.region_id = u.country".
			" left join " . $GLOBALS['ecs']->table('region') . " as r1 on r1.region_id = u.province".
			" left join " . $GLOBALS['ecs']->table('region') . " as r2 on r2.region_id = u.city".
			" left join " . $GLOBALS['ecs']->table('region') . " as r3 on r3.region_id = u.district".
...
function get_user_orders($user_id, $num = 10, $start = 0,$where='')
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
         ...
         $row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. "\" class='btn btn-wide btn-primary'>查看订单</a>";
		
        if ($row['order_status'] == OS_UNCONFIRMED)
        {
            $row['handler'] .= "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\" class='btn btn-wide btn-default'>".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                @$row['handler'] .= "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\" class='btn btn-wide btn-primary'>".$GLOBALS['_LANG']['received']."</a>";
            }
            ...
            else
            {
                ...
                {
                    @$row['handler'] .= "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '" class="btn btn-wide btn-primary">' .$GLOBALS['_LANG']['pay_money']. '</a>';
                }
                else
                {
                    @$row['handler'] .= "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '" class="btn btn-wide btn-primary">' .$GLOBALS['_LANG']['view_order']. '</a>';
                }
			...
			$row['order_status'] == OS_CONFIRMED))
		{
			...
			else if($payment_info['pay_code'] =='MoneyCard')
			{
				$row['pay_online'] = '';
			}
			else if($payment_info['pay_code'] =='wx_new_qrcode')
			{
				$row['pay_online'] = '';
			}
			else
...
function get_order_goods($order)
{
 ...
 /*--wzys ecshop会员订单列表买了的商品直接展示出来end--*/
