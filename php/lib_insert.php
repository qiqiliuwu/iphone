 ...
 * @param   integer $size		每页显示数
 * @param   integer $page		当前页码
 * @param   integer $page_count	总页数
 * @param   integer $more_count	前后总页数
 * @param   string $page_name	页码参数名
 * @return  string
 */
function insert_pager($arr)
{
	$count = isset($arr['count']) ? $arr['count'] : 0;
	$pageSize = isset($arr['size']) ? $arr['size'] : 20;
	$pageId = isset($arr['page']) ? $arr['page'] : 1;
	$pageNum = isset($arr['page_count']) ? $arr['page_count'] : 1;
	$morePages = isset($arr['more_count']) ? $arr['more_count'] : 10;
	$pageName = isset($arr['page_name']) ? $arr['page_name'] : 'page';
	...
	if (!empty($params))
		$baseUrl .= '?'.http_build_query($params).'&'.$pageName.'=';
	else
		$baseUrl .= '?'.$pageName.'=';
	
	$left = $pageId > 1 ? ' <a href="'.$baseUrl.'1">«首页</a> <a href="'.$baseUrl.($pageId - 1).'">上一页</a> ' : ' <span>«首页</span> <span>上一页</span> ';
	$right = $pageId < $pageNum ? ' <a href="'.$baseUrl.($pageId + 1).'">下一页</a> <a href="'.$baseUrl.$pageNum.'">尾页»</a> ' : ' <span>下一页</span> <span>尾页»</span> ';
	
	$pages = '';
	$start = ($pageId - $morePages / 2 < 1) ? 1 : ($pageId - $morePages / 2);
	$end = ($pageId + $morePages / 2 > $pageNum) ? $pageNum : ($pageId + $morePages / 2);
	
	for ($i = $start; $i <= $end; $i++)
		$pages .= ' <a href="'.$baseUrl.$i.'"'.($i == $pageId ? ' class="current"' : '').'>'.$i.'</a> ';
	
	return '<div class="pager">'.$left.$pages.$right.' <span>'.$pageId.'/共'.$pageNum.'页</span></div>';
}
